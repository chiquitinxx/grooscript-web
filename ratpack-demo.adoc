...
application done with http://projects.spring.io/spring-boot/[Spring Boot] is deployed in http://frameworks-groovygs.rhcloud.com/, and
...
buildscript {
  ...
  dependencies {
    ...
    classpath "org.grooscript:grooscript-gradle-plugin:{plugin-version}"
  }
}
...
grooscript {
    ...
    convertDependencies = true //<4>
}

run.dependsOn thread //<5>
--
